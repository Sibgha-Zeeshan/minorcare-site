    ...
    const subscription = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          const { data } = await supabase.from("users").select("*").eq("id", newMessage.sender_id).single()
      ...
      <div className="border-t bg-white shadow-lg">
        <div className="max-w-4xl mx-auto p-4">
          ...
          <div className="mt-3 flex items-center gap-2">
            <AudioRecorder onUpload={handleAudioUpload} disabled={sending || uploadingAudio} />
            {uploadingAudio && <span className="text-xs text-blue-600">Uploading audio...</span>}
          </div>
        </div>
      </div>
    </div>
