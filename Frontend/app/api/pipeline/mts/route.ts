...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/pipeline/mts`, {
      method: "POST",
      headers: {
    ...
    console.error("MTS route failed:", error)
    return NextResponse.json({ error: "MTS pipeline request failed" }, { status: 500 })
  }
}
